<script>
	...
	import { medicationUnitOptions } from '$lib/forms/routine-fields.js';

	let {
		title = $bindable(''),
		time = $bindable(''),
		amount = $bindable(1),
		unitPreset = $bindable('Tablette'),
		unitCustom = $bindable(''),
		titleLabel = 'Medikamentenname',
		titlePlaceholder = 'z. B. Schmerzmittel',
		timeLabel = 'Uhrzeit',
		amountLabel = 'Dosis',
		amountMin = 0.25,
		amountStep = 0.25
